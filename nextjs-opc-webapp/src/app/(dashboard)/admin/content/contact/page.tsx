    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between pb-4 border-b border-border">
        <div>
          ...
          <p className="mt-1 text-xs text-text-muted">
            Configura los textos de encabezado del formulario y el buzón de notificaciones comerciales.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="rounded-xl border border-border bg-surf/50 p-5 space-y-4">
          <div>
            <label className={LABEL}>Etiqueta Superior</label>
            <input defaultValue="CENTRO DE ATENCIÓN GLOBAL" className={INPUT} />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className={LABEL}>Titular del Formulario (ES)</label>
              <input defaultValue="Hablemos de su Próxima Transacción" className={INPUT} />
            </div>
            <div>
              <label className={LABEL}>Titular del Formulario (EN)</label>
              <input defaultValue="Let's Discuss Your Next Energy Allocation" className={INPUT} />
            </div>
          </div>
        ...
        {saved && (
          <div className="flex items-center gap-2 p-3.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold">
            ...
            <span>✓ Configuración de contacto guardada</span>
          </div>
        )}
